...
if (isset($_FILES['image'])) {
    ...
    if (!filter_var($_POST['url'], FILTER_VALIDATE_URL) || (!in_array(parse_url($_POST['url'], PHP_URL_SCHEME),
            $allowed_schemes))
    ) {
        // not a valid URL
...
if (isset($_SESSION['user'])) {
    create_thumb($image_path, $ext, $id, $image);
}
create_thumb(@$image_path, $ext, $id, $image, 'small', 600, 600);
...
function create_thumb($image_path, $ext, $id, $image, $folder = 'thumbs', $max_width = 200, $max_height = 200)
{
    if (isset($_FILES['image'])) {
    ...
    imagejpeg($new_thumb, $folder . '/' . $id . '.jpg', 30);
